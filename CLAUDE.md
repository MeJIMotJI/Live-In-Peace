...
| Visitor counter | ✅ v1 | นับผู้เข้าใช้ทั้งเว็บ — Cloudflare Worker (`src/index.js`) + Durable Object `Counter` (SQLite, singleton `global`) เก็บ `views` (page views รวม ทุกหน้า) กับ `visitors` (unique 1/เครื่อง ครั้งแรกที่เข้าเว็บ ตัดจาก localStorage key `lip-seen` — bump ชื่อ key ใน `counter.js` เมื่ออยากให้เครื่องเก่านับใหม่หลัง reset). `counter.js` โหลดในทุกหน้า (`<script src="/counter.js" defer>` ก่อน `</head>`) ยิง `POST /api/hit {unique}` (ไม่มี keepalive, มี fallback ไป `GET /api/stats` ถ้า POST fail); index.html แสดงยอดใน footer (`#stat-views` / `#stat-visitors`). `GET /api/stats` อ่านอย่างเดียว ไม่บวก. Reset: `GET /api/reset?key=<RESET_KEY>` (secret ตั้งผ่าน `wrangler secret put RESET_KEY` ไม่อยู่ในโค้ด; ไม่ตั้ง = ปิด endpoint). `.assetsignore` กัน src/agents/CLAUDE.md/discord-bot ออกจาก public. Deploy: ดูหัวข้อ 🚀 Deployment (จาก main เท่านั้น) |
