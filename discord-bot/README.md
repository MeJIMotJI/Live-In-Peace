...
1. **`/versetoday`** — ข้อพระคัมภีร์ประจำวัน (ข้อเดียวกันตลอดทั้งวัน) เรียกเองได้ ไม่ได้โพสต์อัตโนมัติแล้ว
2. **`/verse`** — สุ่มข้อพระคัมภีร์ สุ่มใหม่ทุกครั้งที่เรียก
3. **`/read`** — สุ่มบทความจาก [Mustard Seed Community](https://mustardseed.community/) (แพลตฟอร์มคริสเตียนสำหรับคนรุ่นใหม่ ในเครือ Our Daily Bread Ministries) มาให้อ่าน แชร์แค่ title + link + summary สั้นๆ ไม่ copy เนื้อหาเต็ม
...
โพสต์เข้าห้องที่ตั้งไว้ **อัตโนมัติ** ผ่าน Cloudflare Cron Trigger 2 ตัว (ไม่ต้องมีเครื่อง/เซิร์ฟเวอร์เปิดค้างไว้เลย):

- **ทุกวัน 06:00 น. เวลาไทย** — บทเฝ้าเดี่ยววันนี้จาก[พันธกิจมานาประจำวัน](https://www.odbm.org/th/) (การ์ดรูปภาพ +
  ลิงก์ไปอ่านฉบับเต็ม ไม่ได้ดึงเนื้อหาบทความมาโพสต์ซ้ำ เพราะเป็นงานเขียนมีลิขสิทธิ์ของ ODB ที่ต้องขออนุญาตก่อน — ดู
  [Rights and Permissions](https://ourdailybread.org/policy/rights-and-permissions))
- **ทุกวันจันทร์ 06:00 น. เวลาไทย** — เช็ค RSS feed ของ Mustard Seed Community ว่ามีบทความใหม่ในรอบสัปดาห์นี้มั้ย
  ถ้ามีก็แชร์บทความใหม่ ถ้าไม่มีก็สุ่มบทความเก่ามาแชร์แทน (title + link + summary สั้นๆ เหมือน `/read`)
...
กว่าบอทแบบ Gateway — แต่ 4 ฟีเจอร์ปัจจุบันเป็นแบบ "รับคำสั่ง → ตอบ" หรือ "cron → โพสต์" ล้วนๆ จึงเหมาะกับสถาปัตยกรรมนี้
...
เท่านี้บอทก็พร้อมใช้แล้ว ลองพิมพ์ `/verse`, `/versetoday`, `/read` ใน Discord ได้เลย และจะโพสต์เข้าห้องอัตโนมัติ
ทุกเช้า 06:00 น. เวลาไทยด้วย (ทุกวัน = บทเฝ้าเดี่ยว, ทุกวันจันทร์ = บทความ Mustard Seed)
...
## เปลี่ยนเวลาโพสต์ประจำวัน/ประจำสัปดาห์

แก้ค่า `triggers.crons` ใน `wrangler.jsonc` (เป็นเวลา UTC เสมอ — Cloudflare ไม่รองรับ timezone ในนี้ ต้องคำนวณเอง
เช่น อยาก 07:00 เวลาไทย = 00:00 UTC ก็ใส่ `"0 0 * * *"`) มี cron 2 ตัว: ตัวแรก (ทุกวัน) โพสต์บทเฝ้าเดี่ยว ตัวที่สอง
(ทุกวันจันทร์) เช็ค/แชร์บทความ Mustard Seed — ถ้าแก้เวลาของตัวที่สอง ต้องแก้ค่า `WEEKLY_CRON` ใน `src/worker.js` ให้ตรงกัน
ด้วย (ใช้เทียบว่า cron ไหนยิงเข้ามา) แล้ว `npm run deploy` ใหม่
...
└── src/
    ...
    └── lib/
        ...
        ├── mustardseed.js ← ดึงบทความจาก mustardseed.community ผ่าน RSS (title + link + summary)
